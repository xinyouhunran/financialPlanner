<template>
	<view container>
		<view class="register l-flex l-flex-direction l-flex-ai_c">
			...
			<view class="re-phone">
				<view class="">
					<image src="../../static/register-login/invite.png" mode=""></image>
				</view>
				<input type="text" v-model="inviteCode" placeholder="请输入邀请码（选填）" placeholder-class="placeclass"/>
			</view>
				...
				this.$post(url, params, {
					...
					.then(res => {
						if (res.code == 200) {
							...
							.then(data=>{
								if (!res.content.hasSetPassword) {
									let url1 = 'user/center/info/update';								
									let params1 = {
