<template>
	<view class="l-module">
		...
		<view class="l-down-list" :class="{ block: isDownIndex == 0 }">
			<view class="l-module-bg"></view>
			<view class="l-down-view-posi">
				<view class="l-down-view">
					<view class="l-down-item" :class="{ 'l-down-item-active': investStrategyIndex == i }" v-for="(s, i) in investStrategyList"
					 :key="i" @tap="selectTabFn(s, i, 'investStrategyIndex', 'investStrategy')">
						{{ s.title }}
					</view>
				</view>
			</view>
		...
		<view class="l-down-list" :class="{ block: isDownIndex == 1 }">
			<view class="l-module-bg"></view>
			<view class="l-down-view-posi">
				<view class="l-down-view">
					<view class="l-down-item" :class="{ 'l-down-item-active': searchIntervalIndex == i }" v-for="(s, i) in searchIntervalList"
					 :key="i" @tap="selectTabFn(s, i, 'searchIntervalIndex', 'searchInterval')">
						{{ s.title }}
					</view>
				</view>
			</view>
		...
		<view class="l-down-list" :class="{ block: isDownIndex == 2 }">
			<scroll-view scroll-y="true" class="l-scroll-view">
				<view>
					<view v-for="(ss,u) in styleList " :key="u">
						<view class="l-down-head">{{ ss.title }}</view>
						<view class="l-down-view">
							<view class="l-down-item" :class="{ 'l-down-item-active': ss.index == i }" v-for="(s, i) in ss.list" :key="i"
							 @tap="selectTabMuchFn(ss, i)">
								{{ s.title }}
							</view>
						</view>
					</view>
...
<script>
	export default {
		...
		methods: {
			...
			moveHandle() {},
			menuDown(i) {
				if (typeof i === undefined || this.isDownIndex == i) {
					this.isDownIndex = -1;
					return
				}
				this.isDownIndex = i;
...
<style scoped>
	...
	.l-down-list {
		position: fixed;
		top: calc(240rpx + var(--status-bar-height));
		left: 0;
		width: 100%;
		display: none;
		box-sizing: border-box;
